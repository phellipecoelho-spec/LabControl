  ...
  <action>
    ...
    **Actions:**
    ...
    - async logout(allDevices = false): Promise<void>
      - Chama api.post('/auth/logout', { current_password: allDevices ? await getCurrentPassword() : undefined })
...
<task type="auto">
  ...
  <action>
    **1. LoginView.vue:**
    ...
    - Toast: erro 422/401/403, sucesso
    ...
    **4. ResetPasswordView.vue:**
    ...
    - Sucesso -> router.push('/login?reset=1')

    **5. VerifyEmailView.vue (rota /verify-email/:id/:hash):**
    ...
    - Estados: loading -> success -> redirect /login?verified=1 | error -> toast + link "Reenviar"
  ...
  <done>6 views de autenticação criadas com validação, toasts e navegação.</done>
</task>
...
<task type="auto">
  ...
  <action>
    **1. frontend/src/router/routes.ts — adicionar rotas de auth:**
    ...
    { path: '/verify-email/:id/:hash', name: 'verify-email', component: () => import('@/views/auth/VerifyEmailView.vue'), meta: { guest: true, title: 'Verificar Email' } },
    { path: '/verify-email', name: 'verify-email.pending', component: () => import('@/views/auth/VerifyEmailPendingView.vue'), meta: { guest: true, title: 'Verificar Email' } },
    ```
...
- [ ] 6 views: Login, Register, ForgotPassword, ResetPassword, VerifyEmail, VerifyEmailPending
...
<output>
Criar `.planning/phases/02-autenticacao/02-02-SUMMARY.md` quando concluído
</output>