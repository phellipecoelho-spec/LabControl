...
must_haves:
  truths:
    - "POST /api/v1/auth/login retorna cookie de sessão HttpOnly e usuário autenticado"
    - "POST /api/v1/auth/register cria usuário, envia email de verificação, retorna 201"
    - "GET /api/v1/auth/verify-email/{id}/{hash} marca email_verified_at e redireciona para frontend"
    ...
    - "Sanctum configurado para SPA (cookie HttpOnly, CSRF, CORS com credentials)"
  artifacts:
    - backend/app/Http/Controllers/Api/V1/AuthController.php
    - backend/app/Http/Requests/LoginRequest.php
    - backend/app/Http/Requests/RegisterRequest.php
...
<task type="auto">
  ...
  <action>
    ...
    **LoginRequest.php:**
    ...
    - remember (boolean, optional) — se true, gera remember_token de 30 dias (D-01)
...
<task type="auto">
  ...
  <action>
    ...
    **login(LoginRequest $request):**
    ...
    - Se usuário não verificado: return 403 com "Email não verificado" (D-02)
    ...
    **register(RegisterRequest $request):**
    ...
    - Dispara `$user->sendEmailVerificationNotification()` (D-02)
    ...
    **forgotPassword(ForgotPasswordRequest $request):**
    ...
    - Sempre retorna sucesso genérico (não vazar se email existe) (D-03)
    ...
    **resetPassword(ResetPasswordRequest $request):**
    ...
    - Se sucesso: invalida remember_token `$user->remember_token = null; $user->save()` (D-03)
  ...
  <action>
    **1. Atualizar backend/routes/api.php:**
    ...
    Route::prefix('v1')->group(function () {
        ...
        Route::post('/auth/verify-email/{id}/{hash}', [AuthController::class, 'verifyEmail'])->name('verification.verify')->middleware('throttle:auth');
        Route::post('/auth/email/verification-notification', [AuthController::class, 'resendVerification'])->middleware('auth:sanctum', 'throttle:auth');
    ...
    **4. Adicionar RateLimiter no backend/bootstrap/app.php (ou AppServiceProvider):**
    ...
    ```
    E usar `->middleware('throttle:auth')` nas rotas (D-06).
  </action>
  <verify>
    <automated>docker compose exec -T php php artisan route:list --path=api/v1/auth 2>&1</automated>
  </verify>
  ...
  <action>
    ...
    **2. Publicar e customizar ResetPassword:**
    ...
    - Customizar `ResetPassword.php`:
      ...
      - Expiração: 60 minutos (padrão) (D-03)

    **3. Criar templates de email (Markdown) com tema escuro (D-07):**
...
<output>
Criar `.planning/phases/02-autenticacao/02-01-SUMMARY.md` quando concluído
</output>